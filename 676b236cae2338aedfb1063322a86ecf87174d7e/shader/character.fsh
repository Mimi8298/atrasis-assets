...
uniform highp vec3 u_lightIntensity;
...
void main (void)
{
...
#endif
#ifdef SPECULAR_TEX
	spec = texture2D(specularTex, v_texCoord.xy).rgb;
#endif
	...
	// light
#ifdef NORMAL
	vec3 normalSample = texture2D(normalTex, v_texCoord).xyz * 2.0 - vec3(1.0, 1.0, 1.0);
...
#ifdef SPECULAR
	if (ndl > 0.0 && u_lightIntensity.z > 0.0 && dot(spec,spec) > 0.0)
	{
