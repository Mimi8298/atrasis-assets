...
vec4 scTextureLod(sampler2D s, vec2 c, float lod)
{
	...
	vec2 mip = exp2(MAX_SPECULAR_LOD - level + 1.0) / vec2(32.0, TEXTURE_HEIGHT * 0.5);
...
void main (void)
{
...
#ifdef DIFFUSE_TEX
	vec3 albedo = SRGB_TO_LINEAR(texture2D(diffuseTex, v_texCoord.xy).rgb);
#else
	...
	vec3 color = (material.AO * (1.0 - material.METALLIC)) * (1.0 - F) * diffuse.rgb * albedo + specular.rgb * brdf;
...
// debug stuff
	...
	color = t.rgb * (RGBM_MULTIPLIER_SPECULAR * t.a);
#endif
#endif
...
#ifdef PBR
	color = (material.AO * (1.0 - material.METALLIC)) * (1.0 - F) * diffuse.rgb * albedo;
#else
	color = albedo * diffuse.rgb;
#endif
#endif
#ifdef DEBUG_PBR_SPECULAR_TERM
#ifdef PBR
	color = specular.rgb * brdf;
#else
#ifdef SPECULAR
	color = spec * specular;
#endif
